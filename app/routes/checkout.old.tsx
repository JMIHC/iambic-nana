...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const needs100Plus = formData.get("needs100Plus") === "true";
  const isInternational = formData.get("isInternational") === "true";

  // Validate quantity
  if (totalQuantity >= 100 || needs100Plus) {
    return json({ 
      error: "Please contact us directly for orders of 100+ books" 
    }, { status: 400 });
  }

  if (isInternational) {
    return json({ 
      error: "Please contact us for international shipping quotes" 
    }, { status: 400 });
  }

  try {
    const items = JSON.parse(itemsJson);
    const session = await createCheckoutSession(items, totalQuantity, orderNotes);
      ...
      <div className="flex gap-4">
        {canProcessWithStripe && !isInternational ? (
          <Form method="post" className="flex-1">
            ...
            <input type="hidden" name="totalQuantity" value={summary.totalQuantity} />
            <input type="hidden" name="orderNotes" value={orderNotes} />
            <input type="hidden" name="needs100Plus" value={needs100Plus.toString()} />
            <input type="hidden" name="isInternational" value={isInternational.toString()} />
            <button
              type="submit"
              disabled={isProcessing}
              className="w-full bg-purple-600 text-white py-3 px-6 rounded-lg font-bold hover:bg-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
